...
const Routes: React.FC = observer(() => {
    const {auth} = useMst();
  return (
      <div className={`lg:container lg:mx-auto p-5 h-screen flex flex-col items-center content-center`}>
        <Router>
          <Suspense fallback={<div>Loading...</div>}>
            <Switch>
              <Route exact path="/" component={LoginViewEmail} />
              <Route path="/password" component={LoginViewPassword}/>
              <PrivateRoute path="/posts" isAuthenticated={auth.loggedIn} component={PostsView}/>
            </Switch>
          </Suspense>
        </Router>
      </div>
